...
const observer = new MutationObserver(() => {
    ...
    if (header) {
      ...
      if(!injectedHeader) header.insertAdjacentHTML("afterbegin", buildCustomHeader());
    }
...
// Redirect to home when page not found
...
if(!injectErrorPage) sectionParent.insertAdjacentHTML("afterbegin", buildErrorPage());
// window.location.reload()
}
...
window.XMLHttpRequest.prototype.open = function () {
  ...
  if (arguments[1].indexOf('msgstore.' + notionDomain) > -1 || arguments[1].indexOf('msgstore.' + 'www.notion.so') > -1 ) {
    return;
  }
